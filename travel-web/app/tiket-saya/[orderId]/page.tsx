...
import axiosClient from "@/utils/axiosClient"; // Import axiosClient Anda
...
export default function DetailTiket() {
  ...
  useEffect(() => {
    const fetchTiket = async () => {
      ...
      try {
        // Menggunakan axiosClient yang sudah sinkron dengan sistem reservasi
        const response = await axiosClient.get(`/bookings/${orderId}`);
        
        // Menangani struktur data dari Laravel (data.data atau data saja)
        const finalData = response.data.data ? response.data.data : response.data;
        setBooking(finalData);
        
      } catch (error: any) {
        console.error("Error fetching data:", error);
        // Jika tidak authorized (401), arahkan ke login
              ...
              <div className="relative z-10 flex-1 flex flex-col justify-between">
                ...
                <div className="mt-8">
                  <div className={`px-4 py-2 rounded-full text-[10px] font-black shadow-lg border-2 uppercase backdrop-blur-sm text-center
                    ${currentStatus.toLowerCase() === 'success' || currentStatus.toLowerCase() === 'paid' || currentStatus.toLowerCase() === 'settlement'
